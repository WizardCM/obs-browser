...
static void HandleWidgetUnfocus()
{
	...
	for (auto &widget : dockWindows) {
		if (widget->hasFocus()) {
			widget->clearFocus();
			blog(LOG_INFO, "Unfocusing a widget");
		}
	}
}
...
static void InitFocusTracker()
{
	...
	if (!focusHook) {
		...
		// TODO Probably not needed. Will get spammy
		blog(LOG_INFO,
		     "[obs-browser] Initialised browser panel focus override");
	}
#endif
}
...
static void ShutdownFocusTracker()
{
	...
	if (!UnhookWindowsHookEx(focusHook)) {
		...
		// TODO Probably not needed. Will get spammy
		blog(LOG_INFO,
		     "[obs-browser] Browser panel focus override disabled");
	}
#endif
}
	...
	: QCefWidget(parent), url(url_), rqc(rqc_)
{
	...
	//RegisterFocusTrackerWidget(window);
#endif
...
{
//#ifndef __APPLE__
//	UnregisterFocusTrackerWidget(window);
//#endif
